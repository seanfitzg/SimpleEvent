...
Simple.prototype.registerHandler = function (fn) {
  this.commandHandlers[fn.name] = fn;
};
...
Simple.prototype.replay = function (obj, events) {
  ...
  function assignValueFromFunction(currentEvent) {
    var functionName;
    if (typeof currentEvent.command === 'function') {
      functionName = currentEvent.command.name;
    } else {
